        ...
        <v-col cols="12">
          <v-card>
            ...
            <v-card-text>
              <v-row dense>
                <v-col cols="12" md="4">
                  <VisVal label="Descripción" :value="item.offer.description" />
                </v-col>

                <v-col cols="12" md="4">
                  <VisVal
                    label="Tipo de estante"
                    :value="item.offer.stand_type.name"
                  />
                </v-col>
              </v-row>
            </v-card-text>
...
const getItem = async () => {
  ...
  try {
    const endpoint = `${URL_API}/v1/company/events/stand_requests/${itemId.value}`;
