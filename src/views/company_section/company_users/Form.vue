...
const getCatalogs = async () => {
  try {
    const endpoint = `${URL_API}/v1/company/catalogs/roles`;
...
const getItem = async () => {
  ...
  try {
    const endpoint = `${URL_API}/v1/company/users/${itemId.value}`;
    const response = await axios.get(endpoint, authHdrs());
...
const handleAction = async () => {
  ...
  try {
    ...
    const endpoint = `${URL_API}/v1/company/users${
